    ...
    for (const dir of directories) {
      ...
      if (fs$1.existsSync(gitPath)) {
        try {
          ...
          let cmd = `git -C "${repoPath}" log --all -i --author="${localAuthor}" --pretty=format:"%h|%ad|%s|%D" --date=short`;
          if (since) {
            cmd += ` --since="${since}"`;
          }
          if (until) {
...
app$1.whenReady().then(() => {
  ...
  ipcMain$1.handle("save-settings", (event, key, value) => {
    store.set(key, value);
  });
  ipcMain$1.handle("scan-repos", async (event, options) => {
