...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }) => {
  return (
    <div
      className="w-64 glass flex flex-col border-r border-color h-full"
      style={{ borderRight: '1px solid var(--border-color)' }}
    >
      <div
        className="p-6 flex items-center gap-3 border-b border-color"
...
const NavItem: React.FC<{
  ...
  <button
    ...
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
      active ? 'bg-blue-500/10 text-blue-400' : 'hover:bg-white/5 text-secondary hover:text-primary'
    }`}
    style={{
